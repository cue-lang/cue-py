...
class Value:
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if isinstance(other, Value):
            return libcue.is_equal(self.res(), other.res())
        return False
    ...
    def unify(self, other: 'Value') -> 'Value':
        ...
        """
        v = libcue.unify(self.res(), other.res())
    ...
    def kind(self) -> Kind:
        ...
        """
        return to_kind[libcue.concrete_kind(self.res())]
    ...
    def check_schema(self, schema: 'Value', *opts: EvalOption) -> None:
        ...
        err = libcue.instance_of(self.res(), schema.res(), eval_opts)
        if err != 0:
            raise Error(err)
    ...
    def validate(self, *opts: EvalOption) -> None:
        ...
        """
        eval_opts = encode_eval_opts(*opts)
        err = libcue.validate(self.res(), eval_opts)
        if err != 0:
            raise Error(err)

def _to_int(val: Value) -> int:
    ptr = libcue.ffi.new("int64_t*")
    err = libcue.dec_int64(val.res(), ptr)
    if err != 0:
        raise Error(err)
    return ptr[0]
...
def _to_unsigned(val: Value) -> int:
    ...
    err = libcue.dec_uint64(val.res(), ptr)
    if err != 0:
        raise Error(err)
    return ptr[0]
...
def _to_bool(val: Value) -> bool:
    ...
    err = libcue.dec_bool(val.res(), ptr)
    if err != 0:
        raise Error(err)
    return ptr[0]
...
def _to_float(val: Value) -> float:
    ...
    err = libcue.dec_double(val.res(), ptr)
    if err != 0:
        raise Error(err)
    return ptr[0]
...
def _to_str(val: Value) -> str:
    ...
    err = libcue.dec_string(val.res(), ptr)
    if err != 0:
        raise Error(err)
...
def _to_bytes(val: Value) -> bytes:
    ...
    err = libcue.dec_bytes(val.res(), buf_ptr, len_ptr)
    if err != 0:
        raise Error(err)
...
def _to_json(val: Value) -> str:
    ...
    err = libcue.dec_json(val.res(), buf_ptr, len_ptr)
    if err != 0:
        raise Error(err)
...
def _lookup(val: Value, path: str) -> Value:
    ...
    err = libcue.lookup_string(val.res(), path_ptr, val_ptr)
    if err != 0:
        raise Error(err)
...
def _default(val: Value) -> Optional[Value]:
    ...
    res = libcue.default(val.res(), ok_ptr)
    if ok_ptr[0] == 1:
